...
- Core View Transition helpers live in `components/view-transitions.tsx`; core `::view-transition-*` rules live in `styles/view-transitions.css`.
- Title View Transition names use `<kind>-title-<slug>` via `pageTitleTransitionName`; core kinds are `project` and `post`.
- The `site-header` and `site-footer` View Transition groups are frozen during route transitions, while `components/scroll-reset.tsx` remains required for scroll restoration.
- Playground index, card-grid, and directional View Transition work is intentionally deferred to [Plan 021](./plans/021-playground-index-view-transitions.md).
...
- Last checked `master` commit: `e37447a3d6e8`
- Last checked at: 2026-07-03T00:12:07-0700
...
- Latest run note: 0 major issues; latest score stayed 58/100 after merging PR #19 and PR #20, with 478 warning diagnostics. The latest log lists the top 10 warning rule groups by violation count. Keep the explicit `PATH` in the command because the absolute `npx` entrypoint uses `/usr/bin/env node`, and non-login automation shells may not otherwise resolve `node`.
