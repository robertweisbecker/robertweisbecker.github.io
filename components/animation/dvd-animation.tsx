...
export function DvdAnimationRoot({
  ...
  const handleCornerHit = React.useCallback(() => {
    setCornerHits((currentCornerHits) => currentCornerHits + 1);
  }, []);

  React.useEffect(() => {
    if (cornerHits <= previousReportedCornerHitsRef.current) return;

    previousReportedCornerHitsRef.current = cornerHits;
    onCornerHit?.(cornerHits);
  }, [cornerHits, onCornerHit]);
  ...
  React.useEffect(() => {
    ...
    function animate(currentTime: number) {
      ...
      if (logo) {
        const color = getCurrentLogoColor(elapsed, xDuration, yDuration, colors);
...
export type DvdAnimationStageProps = React.ComponentProps<"svg"> & {
  logoViewBox?: string;
};

export function DvdAnimationStage({
  className,
  children,
  logoViewBox = `0 0 ${LOGO_VIEW_BOX_WIDTH} ${LOGO_VIEW_BOX_HEIGHT}`,
  ...props
}: DvdAnimationStageProps) {
  ...
  return (
    <svg
      ...
      {...props}
    >
      <svg ref={logoRef} width={logoWidth} height={logoHeight} viewBox={logoViewBox}>
        {children ?? <path fill="inherit" d={LOGO_PATH} />}
      </svg>
    </svg>
  );
}
...
export function DvdAnimationScore({
  ...
  React.useEffect(() => {
    return () => {
      balloonTimeoutsRef.current.forEach((timeoutId) => {
  ...
  return (
    <>
      <div
        ref={scoreRef}
        className={cn("relative flex min-h-button-xs items-center gap-1 px-1.5 text-center text-foreground", className)}
        {...props}
      >
