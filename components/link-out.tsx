...
export function LinkOut({ text, href, src, className, linkClass }: LinkOutProps) {
  return (
    <span className={cn("group/link inline items-center space-x-1 self-baseline align-baseline", className)}>
      {src && (
        <img
          src={src}
          alt={`${text} Logo`}
      ...
      <a
        ...
        className={cn("link relative inline-flex items-center gap-[0.25em]", linkClass)}
      >
        {text}
        <LinkOutIcon className="absolute top-px -right-[.25em] -my-0.5 -me-0.5 size-[.75em] opacity-0 transition-[translate,opacity] group-hover/link:translate-x-[0.125em] group-hover/link:-translate-y-[0.125em] group-hover/link:opacity-100" />
      </a>
    </span>
  );
}
