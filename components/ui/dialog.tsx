...
function DialogPopup({
  ...
  return (
    <DialogPrimitive.Popup
      ...
      className={cn(
        "relative z-50 grid w-full max-w-dialog gap-4 overflow-hidden rounded-2xl bg-card p-2 text-sm shadow-border-xl duration-100 sm:max-w-lg sm:p-4 data-open:animate-in data-open:fade-in-0 data-open:zoom-in-95 data-closed:animate-out data-closed:fade-out-0 data-closed:zoom-out-95",
        className
      )}
      {...props}
    >
    ...
    <DialogPortal keepMounted={keepMounted}>
      ...
      <DialogPrimitive.Viewport className="group/dialog fixed inset-0 z-50">
        ...
        >
          <ScrollArea.Viewport
            className={cn(
              "h-full overscroll-contain group-data-ending-style/dialog:pointer-events-none",
                ...
                {showCloseButton && (
                  <DialogPrimitive.Close
                    ...
                    render={
                      <Button
                        variant="ghost"
                        className="absolute inset-e-3 top-3 rounded-full bg-muted"
                        size="icon-sm"
                      />
                    }
                  >
...
function DialogBody({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div data-slot="dialog-body" className={cn("flex flex-col gap-2 px-2 pb-2 text-base", className)} {...props} />
  );
}
...
function DialogDescription({ className, ...props }: DialogPrimitive.Description.Props) {
  ...
  export {
    ...
    DialogDescription,
    DialogFooter,
    DialogHeader,DialogOverlay,DialogPopup,DialogPortal,
    DialogTitle,
    DialogTrigger
  }
