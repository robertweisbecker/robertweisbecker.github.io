...
function SelectTrigger({
  ...
  return (
    <SelectPrimitive.Trigger
      ...
      data-size={size}
      className={cn(
        "group/select-trigger flex w-fit items-center justify-between gap-1.5 truncate rounded-md bg-card py-2 ps-2.5 pe-2 text-sm whitespace-nowrap text-foreground shadow-border-sm outline outline-transparent transition-[background-color,color,box-shadow,outline] select-none squircle hover:bg-popover hover:not-data-disabled:outline-border focus-visible:outline-2 focus-visible:outline-ring disabled:cursor-not-allowed disabled:opacity-50 aria-invalid:border-destructive aria-invalid:ring-3 aria-invalid:ring-destructive/50 data-placeholder:text-muted-foreground/50 data-[size=default]:h-button data-[size=sm]:h-button-sm data-[size=sm]:rounded-sm *:data-[slot=select-value]:flex *:data-[slot=select-value]:items-center *:data-[slot=select-value]:gap-1.5 dark:bg-current/5 dark:-outline-offset-1 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
        className
      )}
      {...props}
    >
...
function SelectItem({ className, children, ...props }: SelectPrimitive.Item.Props) {
  return (
    <SelectPrimitive.Item
      ...
      className={cn(
        "group/select-item relative flex w-full items-center gap-2 rounded-md py-2 ps-7 pe-3 text-sm/none text-popover-foreground outline-none select-none squircle",
