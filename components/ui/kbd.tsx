...
const kbdVariants = cva(
  "inline-flex items-center justify-center gap-1 rounded select-none [&_svg:not([class*='size-'])]:size-3 shrink-0 relative whitespace-nowrap font-mono",
  {
    variants: {
      variant: {
        default:
          "bg-current/10 font-pixel text-[11px] font-medium text-muted-foreground in-data-[slot=tooltip-content]:bg-background/20 in-data-[slot=tooltip-content]:text-background dark:in-data-[slot=tooltip-content]:bg-background/10 px-1 h-lh w-fit min-w-lh",
        elevated: [
