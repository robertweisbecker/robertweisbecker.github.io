...
export function AutocompleteInput({
  ...
  return (
    <AutocompletePrimitive.InputGroup
      className={cn(inputVariants({ size: sizeValue }), "relative w-full text-foreground has-disabled:opacity-64")}
      data-slot="autocomplete-input-group"
    >
      {startAddon && (
        <div
          aria-hidden="true"
          className="pointer-events-none absolute inset-y-0 start-px z-10 flex items-center ps-2.5 [&_svg]:-mx-0.5 [&_svg:not([class*='size-'])]:size-4"
      ...
      <AutocompletePrimitive.Input
        className={cn(
          startAddon &&
            "ps-[calc(--spacing(8.5)-1px)] data-[size=sm]:ps-[calc(--spacing(7.5)-1px)] sm:ps-[calc(--spacing(8)-1px)] sm:data-[size=sm]:ps-[calc(--spacing(7)-1px)]",
          sizeValue === "sm"
...
export function AutocompleteList({ className, ...props }: AutocompletePrimitive.List.Props): React.ReactElement {
  return (
    <ScrollArea scrollbarGutter scrollFade className="max-h-full flex-1">
...
export function AutocompleteClear({ className, ...props }: AutocompletePrimitive.Clear.Props): React.ReactElement {
  return (
    <AutocompletePrimitive.Clear
      className={cn(
        "absolute inset-e-0.5 top-1/2 inline-flex size-button-sm shrink-0 -translate-y-1/2 items-center justify-center rounded-md transition-[color,background-color,box-shadow,opacity] hover:opacity-100 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
        className
      )}
