...
function ChromeTabsRoot({ className, children, ...props }: TabsPrimitive.Root.Props) {
  return (
    <TabsPrimitive.Root
      className={cn(
        "flex w-full flex-col rounded-[calc(var(--radius-lg)+(--spacing(0.5)))] bg-muted p-0.5 drop-shadow-xs [&>div]:data-[slot=tabs-panel]:bg-card",
        className
      )}
      {...props}
    >
...
function ChromeTabsTab({ value, children, className, flush = true, ...props }: TabsPrimitive.Tab.Props & { flush?: boolean }) {
  return (
    <TabsPrimitive.Tab
      ...
      {...props}
    >
      <span className="flex flex-1 items-center gap-2 truncate px-4 py-1 before:absolute before:inset-1 before:-z-1 before:rounded-md before:transition-colors group-hover/tab:not-group-data-active/tab:before:bg-accent group-active/tab:before:bg-transparent group-data-active/tab:before:bg-transparent">
        {children ?? value}
      </span>
    </TabsPrimitive.Tab>
  );
